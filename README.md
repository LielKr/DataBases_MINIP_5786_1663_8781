...
1. [GUESTS – אורחים](#1-guests--אורחים)
2. [ROOM_TYPES – סוגי חדרים](#2-room_types--סוגי-חדרים)
3. [ROOMS – חדרים](#3-rooms--חדרים)
4. [BOOKING_SOURCES – מקורות הזמנה](#4-booking_sources--מקורות-הזמנה)
5. [BOOKINGS – הזמנות](#5-bookings--הזמנות)
6. [ROOM_ASSIGNMENTS – שיוך חדרים להזמנות](#6-room_assignments--שיוך-חדרים-להזמנות)
7. [CHECK_INS_OUTS – תיעוד כניסה ויציאה בפועל](#7-check_ins_outs--תיעוד-כניסה-ויציאה-בפועל)
...
כל רשומה בטבלה מייצגת אורח יחיד.
...
|---|---|---|---|---|
| `guest_id` | `INT` | כן | מספר מזהה ייחודי לכל אורח | מפתח ראשי |
...
| `registration_date` | `DATE` | כן | התאריך שבו האורח נרשם לראשונה במערכת | — |
...
## 2. `ROOM_TYPES` — סוגי חדרים

### תיאור הטבלה

טבלת `ROOM_TYPES` מגדירה את קטגוריות החדרים הקיימות במלון, לדוגמה: חדר יחיד, חדר זוגי, חדר משפחתי או סוויטה.

במקום לשכפל מידע זהה עבור כל חדר, פרטי סוג החדר נשמרים פעם אחת בטבלה זו, וכל חדר משויך לסוג המתאים לו.  
כך ניתן לשנות מחיר בסיס או תיאור של קטגוריה במקום אחד בלבד.

### שדות הטבלה

| שם השדה | סוג נתון | חובה | תיאור | אילוצים |
|---|---|---|---|---|
| `type_id` | `INT` | כן | מספר מזהה ייחודי לסוג החדר | מפתח ראשי |
| `type_name` | `VARCHAR(50)` | כן | שם הקטגוריה, לדוגמה: זוגי, סוויטה | — |
| `base_price` | `NUMERIC(10,2)` | כן | מחיר בסיס ללילה בשקלים | חייב להיות גדול מ־0 |
...
הטבלה שומרת מידע על מספר החדר, הקומה שבה הוא נמצא, מצבו הנוכחי, השלוחה הטלפונית, סוג הנוף, וסוג החדר שאליו הוא שייך.

### שדות הטבלה

| שם השדה | סוג נתון | חובה | תיאור | אילוצים |
|---|---|---|---|---|
| `room_id` | `INT` | כן | מזהה ייחודי לכל חדר | מפתח ראשי |
| `room_number` | `VARCHAR(10)` | כן | מספר החדר כפי שמוצג במלון | ייחודי |
...
| `view_type` | `VARCHAR(30)` | לא | סוג הנוף הנשקף מהחדר | — |
...
## 4. `BOOKING_SOURCES` — מקורות הזמנה
...
| שם השדה | סוג נתון | חובה | תיאור | אילוצים |
|---|---|---|---|---|
| `source_id` | `INT` | כן | מספר מזהה ייחודי למקור ההזמנה | מפתח ראשי |
...
- הזמנה אחת יכולה לכלול חדרים רבים.
- להזמנה יכולה להיות רשומת כניסה ויציאה בפועל.
...
| שם השדה | סוג נתון | חובה | תיאור | אילוצים |
|---|---|---|---|---|
| `assignment_id` | `INT` | כן | מספר שיבוץ ייחודי | מפתח ראשי |
...
## 7. `CHECK_INS_OUTS` — תיעוד כניסה ויציאה בפועל

### תיאור הטבלה

טבלת `CHECK_INS_OUTS` שומרת את נתוני הכניסה והיציאה בפועל של אורחים בדלפק הקבלה.

הטבלה מאפשרת להשוות בין תאריכי השהייה שתוכננו בהזמנה לבין מה שקרה בפועל.
...
| שם השדה | סוג נתון | חובה | תיאור | אילוצים |
|---|---|---|---|---|
| `log_id` | `INT` | כן | מספר מזהה לרשומת כניסה או יציאה | מפתח ראשי |
| `booking_id` | `INT` | כן | ההזמנה שאליה שייכת הרשומה | מפתח זר אל `BOOKINGS(booking_id)` |
| `actual_check_in` | `DATE` | לא | תאריך הכניסה בפועל | — |
| `actual_check_out` | `DATE` | לא | תאריך היציאה בפועל | — |

### קשרים

- זוהי ישות חלשה התלויה בטבלת `BOOKINGS`.
- רשומת כניסה או יציאה לא יכולה להתקיים ללא הזמנה קיימת.
...
| קשר | סוג קשר | הסבר |
|---|---|---|
| `GUESTS` → `BOOKINGS` | 1:N | אורח אחד יכול לבצע מספר הזמנות |
| `BOOKING_SOURCES` → `BOOKINGS` | 1:N | מקור הזמנה אחד יכול להיות משויך להזמנות רבות |
| `ROOM_TYPES` → `ROOMS` | 1:N | סוג חדר אחד יכול לתאר מספר חדרים |
